...
# qsub -cwd -o logs/2_QC_and_Index_HsMm_QSUB_$(date +%Y%m%d).log -e logs/2_QC_and_Index_HsMm_QSUB_$(date +%Y%m%d).error -S /bin/bash -V -N Submit_2 -q geschwind.q -l h_data=4G,h_rt=1:00:00 2_QC_and_Index_HsMm_QSUB.sh

# Reminder: make /logs directory in code directory
################################################################################
echo ""
echo "Starting 2_QC_and_Index_HsMm_QSUB.sh..."$(date)
echo ""
################################################################################
...
for sampleDir in ${inLaneDir}/*; do
	...
	if [ ! -f ${outFpath} ]; then
		...
		qsub -cwd -o ${outLog} -e ${outErr} -V -S /bin/bash -q geschwind.q -N QC_${bnSampleDir} -l h_data=64G,h_rt=12:00:00 -pe shared 8 2_QC_and_Index_HsMm.sh ${inParentDir} ${outDir} ${bnSampleDir}
...
echo ""
echo "End of 2_QC_and_Index_HsMm_QSUB.sh... "$(date)
