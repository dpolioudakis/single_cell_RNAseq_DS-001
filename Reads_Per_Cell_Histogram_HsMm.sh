...
# qsub -cwd -o logs/Reads_Per_Cell_Histogram_HsMm_$(date +%Y%m%d).log -e logs/Reads_Per_Cell_Histogram_HsMm_$(date +%Y%m%d).error -S /bin/bash -V -N DRH_QSUB -q geschwind_short.q Reads_Per_Cell_Histogram_HsMm.sh

# Reminder: make /logs directory in code directory

################################################################################
echo ""
echo "Starting Reads_Per_Cell_Histogram_HsMm.sh..."$(date)
echo ""
################################################################################
...
echo ""
echo "End of Reads_Per_Cell_Histogram_HsMm.sh... "$(date)
